...
const updateCartDOM = (state) => {
    ...
    if (state.items.length === 0) {
        ...
        // Render items with stagger effect if newly added
...
const updateProductButtons = (items) => {
    products.forEach(product => {
        ...
        if (itemInCart) {
            ...
            // Check if it was previously a pill to avoid unnecessary re-renders
            if (container.querySelector('.pill-selector')) {
                container.innerHTML = `
                    <button class="add-to-cart w-full py-4 bg-white/5 hover:bg-white/10 rounded-2xl font-bold transition-all flex items-center justify-center gap-2" data-id="${product.id}">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
                        </svg>
                        Add to Cart
...
cardNumberInput.addEventListener('input', (e) => {
    ...
    } else if (value.startsWith('34') || value.startsWith('37')) { // Amex
...
document.getElementById('close-receipt').addEventListener('click', () => {
    location.reload(); // Refresh for clean state
});
